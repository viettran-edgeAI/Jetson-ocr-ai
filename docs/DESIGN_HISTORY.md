...
| Date | Change | Why |
| --- | --- | --- |
| 2026-08-10 | Replaced the split upload/output browser layout with a full-screen dark-first unified chat workspace: far-left sessions, stacked Answer/Translate and Run OCR/Search web actions, sequential multi-file attachments, ordered OCR messages, backend-only Brave grounding, and optional gated multimodal image forwarding. | The current browser and service contracts treat OCR artifacts, chat, translation, and web-supported answers as one session flow. Ordered attachment storage preserves legacy sessions, while bounded context/image forwarding and an optional `LLM_MMPROJ_PATH` keep model input explicit; the current host still needs compatible mmproj weights for visual inference. |
